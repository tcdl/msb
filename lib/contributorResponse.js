var _ = require('lodash');
var util = require('util');
var http = require('http');
var ServerResponse = http.ServerResponse;

function ContributorResponse(contributor) {
  this.contributor = contributor;
  this.body = null;
  this.finished = false;

  /* start Core compatibility */
  this._hasBody = (contributor.originalMessage.payload.method === 'HEAD') ? false : true;
  this._header = null;
  this._headers = null;
  this._headerNames = {};
  /* end Core compatibiltiy */
}

var response = ContributorResponse.prototype;

// Inherit from Core
_.extend(response, _.pick(ServerResponse.prototype,
  'statusCode',
  'statusMessage',
  'writeHead',
  'setHeader',
  'getHeader',
  'removeHeader',
  '_renderHeaders'
));

response.end = function(body, cb) {
  if (!cb && typeof body === 'function') {
    cb = body;
    body = this.body;
  } else {
    body = body || this.body;
  }

  if (this.finished) {
    return false;
  }

  if (!this._header) {
    this.writeHead(this.statusCode);
  }

  var payload = {
    statusCode: this.statusCode,
    statusMessage: this.statusMessage,
    body: null
  };

  if (this._header) {
    payload.headers = this._header;
  }

  if (body && this._hasBody) {
    if (body instanceof Buffer) {
      payload.bodyBuffer = body.toString('base64');
    } else {
      payload.body = body;
    }
  }

  this.finished = true;
  return this.contributor.send(payload, cb);
};

response._storeHeader = function(statusLine, renderedHeaders) {
  /* Core safety  */
  this._header = renderedHeaders;
};

module.exports = ContributorResponse;
