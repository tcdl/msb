...
collector._setContribsRemainingForContributorId = function(contributorId, contribsRemaining) {
  ...
  if (contribsRemaining >= 0) {
    // Set
    _contribsRemainingById[contributorId] = contribsRemaining;
  } else {
    // Decrement
    _contribsRemainingById[contributorId] = Math.max(0, _contribsRemainingById[contributorId] + contribsRemaining);
  }
