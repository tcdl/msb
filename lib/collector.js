...
function Collector(config) {
  ...
  if ('waitForResponses' in config && config.waitForResponses !== -1) {
    ...
    this.waitForResponses = Number.MAX_VALUE;
  }
...
collector._onResponseMessage = function(shouldAcceptMessageFn, message) {
  ...
  this.emit('response', message.payload, message);
...
collector._setResponsesRemainingForResponderId = function(responderId, responsesRemaining) {
  if (this._responsesRemainingById && this._responsesRemainingById[responderId] === responsesRemaining) return null; // Not changed
  if (responsesRemaining < 0 && (!this._responsesRemainingById || !this._responsesRemainingById[responderId])) return null; // Reached min

  var _responsesRemainingById = this._responsesRemainingById = this._responsesRemainingById || {};
  _responsesRemainingById[responderId] = (responsesRemaining === 0) ? 0 : Math.max(0, (_responsesRemainingById[responderId] || 0) + responsesRemaining);
