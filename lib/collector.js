...
collector.listenForResponses = function (topic, shouldAcceptMessageFn) {
    if (this.listeners('error').length < 2) {
        logger.warn('A Collector_ \'error\' event handler should be implemented.');
    }
    ...
    if (this.waitForAcksMs)
        this.waitForAcksUntil = new Date(this.startedAt.valueOf() + this.waitForAcksMs);
    return this;
};
collector.removeListeners = function () {
    if (this.responseChannel)
        this.responseChannel.removeListener('message', this._onResponseMessage);
};
...
collector._onResponseMessage = function (shouldAcceptMessageFn, message) {
    ...
    if (this.isCanceled)
        return; // e.g. error occurs during event
    this._processAck(message.ack);
    if (this.isAwaitingResponses())
        return;
    if (this.isAwaitingAcks())
        return this._enableAckTimeout();
    this.end();
};
collector._processAck = function (ack) {
    if (!ack)
        return; // `null` ack is valid
...
collector._getMaxTimeoutMs = function () {
    ...
    for (var key in this._timeoutMsById) {
        ...
        if (responsesRemainingById && (key in responsesRemainingById) && !responsesRemainingById[key])
            continue;
...
collector._getResponsesRemaining = function () {
    if (!this._responsesRemainingById)
        return this._responsesRemaining;
...
collector._setTimeoutMsForResponderId = function (responderId, timeoutMs) {
    ...
    if (_timeoutMsById[responderId] === timeoutMs)
