...
/* Kafka Settings */
// config.kafka = {
//   connectionString: '172.16.131.3:2181',
//   consumerOptions: {
...
/* Redis Settings (Default - other adapters take precedence) */
config.redis = {
  port: 6379,
  host: '127.0.0.1',
  options: {}
...
config._afterConfigure = function() {
  ...
  if (config.kafka && !config.kafka.groupId) config.kafka.groupId = config.serviceDetails.name;
};
