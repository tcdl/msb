...
brokerDefaults.amqp = {
  host: '127.0.0.1',
  port: 5672,
  groupId: null // Will be `config.serviceDetails.name`
};

/* Kafka */
brokerDefaults.kafka = {
...
config._afterConfigure = function() {
  config[config.brokerAdapter] = _.defaults(config[config.brokerAdapter] || {}, brokerDefaults[config.brokerAdapter]);
