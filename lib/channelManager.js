...
channelManager.create = function() {
  ...
  channelManager.createRawProducer = function(topic, options, callback) {
    var a = getAdapter(); // Adapter must be loaded before config is used

    var producerConfig = _.merge(adapterConfig, options);

    return a.Publish(producerConfig).channel(helpers.validatedTopic(topic));
  };
  ...
  function getAdapter() {
    ...
    adapter = require(ADAPTER_PATHS.activemq).create();
