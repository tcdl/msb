...
channelManager.create = function() {
  ...
  function getAdapter() {
    ...
    adapter = require(ADAPTER_PATHS[config.brokerAdapter]).create(config);

    if (adapter.on) {
      adapter.on('connectionTimeout', channelManager.emit.bind(channelManager, 'connectionTimeout'));
    }
