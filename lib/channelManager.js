...
channelManager.create = function () {
    ...
    channelManager.findOrCreateConsumer = function (topic, options) {
        ...
        function onMessage(message) {
            ...
            if (config.autoMessageContext)
                messageFactory.startContext(message);
            channel.emit('message', message);
            if (config.autoMessageContext)
                messageFactory.endContext();
            if (autoConfirm)
                channel.confirmProcessedMessage(message, true);
        }
        ...
        if (isServiceChannel || !config.cleanupConsumers)
            return channel;
        channel.on('removeListener', function (eventName) {
            if (eventName !== 'message')
                return;
            if (~consumerTopicsToCheck.indexOf(topic) || channel.listeners(eventName).length)
                return;
            consumerTopicsToCheck.push(topic);
            if (consumerTopicsToCheck.length > 1)
                return;
    ...
    function getAdapter() {
        ...
        if (!adapterConfig)
            throw new Error('Invalid broker adapter \'' + config.brokerAdapter + '\'');
    ...
    function checkConsumers() {
        consumerTopicsToCheck.forEach(function (topic) {
            var channel = consumersByTopic[topic];
            if (channel.listeners('message').length)
