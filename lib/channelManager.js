...
channelManager.create = function() {
  ...
  channelManager.findOrCreateConsumer = function(topic, options) {
    ...
    var autoConfirm = config.autoConfirm || options && options.autoConfirm;
    ...
    channel.confirmProcessedMessage = (channel.raw.confirmProcessedMessage) ? function(message) {
      channel.raw.confirmProcessedMessage(message);
    } : noop;

    function onMessage(message) {
      if (messageHasExpired(message)) {
        if (!autoConfirm) channel.rejectMessage(message);
        return;
      }
      if (autoConfirm) channel.confirmProcessedMessage(message);
