...
channelManager.createConsumer = function(topic) {
  var adapter = queue();
  var subscriberConfig = _.merge({ channel: helpers.validatedTopic(topic) }, adapterConfig);
  return adapter.Subscribe(subscriberConfig);
};

function queue() {
  if (config.kafka) {
    adapterConfig = config.kafka;
    return require('./adapters/kafka');
  }
  if (config.amqp) {
    adapterConfig = config.amqp;
    return require('message-queue')('amqp');
  }
  adapterConfig = config.redis;
  return require('./adapters/redis'); // Redis
}
