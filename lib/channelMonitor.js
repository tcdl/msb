...
channelMonitor.startBroadcasting = function() {
  ...
  heartbeatContributorListener = Contributor.attachListener({
    ...
    contributor.send({ body: channelMonitor.localInfoByTopic });
  });
...
channelMonitor.doHeartbeat = function() {
  ...
  originator.on('contrib', function(message) {
    aggregateChannelInfo(collectingInfoByTopic, message.payload.body, message.meta.serviceDetails.instanceId);
  });
  ...
  originator.message.payload.body = collectingInfoByTopic;
  originator.publish();
};
