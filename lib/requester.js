...
var Collector_ = require('./collector');
function Requester(config, originalMessage) {
    if (!(this instanceof Requester))
        return new Requester(config, originalMessage);
    ...
    requester.apply(this, arguments);
}
util.inherits(Requester, Collector_);
...
requester._publish = function () {
    ...
    self
        ...
        if (!self.isAwaitingAcks() && !self.isAwaitingResponses())
            return self.end();
        self.enableTimeout();
    });
    return self;
